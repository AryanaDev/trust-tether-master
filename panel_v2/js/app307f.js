...
for (let link of links) {
  link.addEventListener("click", (e) => {
    ...
    const article = document.querySelector(
      `.scrollin:nth-of-type(${e.target.dataset.id})`
    );
...
$(".addbalance").on("click", function () {
  ...
  $(this).closest("form").find(".coinvalue").valid();
...
$(document).on("click", ".vpric-item", function () {
  setTimeout(() => {
    ...
    $(this).closest("form").find(".numto-fa").valid();
    if ($(this).closest("form").validate().checkForm()) {
...
$(".copy-val").on("click", function () {
  ...
  $inputValue.val($(inputCopy).val()).select();
  document.execCommand("copy");
  $inputValue.remove();
  cuteToast({
    type: "success",
    title: "با موفقیت کپی شد",
    message: $(inputCopy).val(),
    timer: 3500,
  });
});
...
$("td[data-pricedollar]").each(function () {
  var tdDollarPrice = $(this).find("strong").text();
  ...
  if (firstFourDigits.length === 4) {
    if (irancell.some((code) => code === firstFourDigits)) {
      ...
      cuteToast({
        ...
        title: "پیام سیستم",
        message:
          "شماره وارد شده متعلق به اپراتورهای ایرانسل و همراه اول نمی‌باشد",
        timer: 5000,
      });
    }
  }
...
$(document).on("click", ".lastmob", function () {
  ...
  $(this)
    ...
    .addClass("valid");
  setTimeout(() => {
    $(this).closest("form").find(".mobinput").valid();
    if ($(this).closest("form").validate().checkForm()) {
      $(this).closest("form").find(".submitbtn").attr("disabled", false);
    }
  }, 0);
...
function formatAmount(input) {
  ...
  // جدا کردن اعداد به گروه‌های سه‌تایی
  const formattedValue = value.replace(/\B(?=(\d{3})+(?!\d))/g, ',');
  
  // تنظیم مقدار جدید به ورودی
