    ...
    t.extend(t.expr.pseudos || t.expr[":"], {
        ...
        t.extend(t.validator, {
            ...
            messages: {
                ...
                maxlength: t.validator.format("لطفا بیشتر از {0} کارکتر وارد کنید."),
                minlength: t.validator.format("حداقل می بایست {0} کارکتر وارد نمایید."),
                rangelength: t.validator.format("لطفاً مقداری بین {0} تا {1} کاراکتر وارد کنید."),
                range: t.validator.format("لطفاً مقداری بین {0} و {1} وارد کنید."),
                max: t.validator.format("لطفاً مقداری کمتر یا مساوی با {0} وارد کنید."),
                min: t.validator.format("لطفاً مقداری بزرگتر یا مساوی با {0} وارد کنید."),
...
$.validator.addMethod("minpric", function (value, element) {
    let minPric = parseFloat(value.replace(/[^0-9.]/g, ''));
    if (minPric < 50000) {
        $.validator.messages.minpric = 'مبلغ واریزی نمی تواند کمتر از ۵۰ هزار تومان باشد';
        return false;
    } else if (minPric > 25000000) {
        $.validator.messages.minpric = 'مبلغ واریزی نمی تواند بیشتر از ۲۵ میلیون تومان باشد';
        return false;
    }
    return true;
}, function (value, element) {
...
$.validator.addMethod("withdrawpric", function (value, element) {
    let withdrawpric = parseFloat(value.replace(/[^0-9.]/g, ''));
    if (withdrawpric < 50000) {
        $.validator.messages.withdrawpric = 'مبلغ قابل برداشت نمی تواند کمتر از ۵۰ هزار تومان باشد';
        return false;
    } else if (withdrawpric > 50000000) {
        $.validator.messages.withdrawpric = 'مبلغ قابل برداشت نمی تواند بیشتر از ۵۰ میلیون تومان باشد';
        return false;
    }
    return true;
}, function (value, element) {
